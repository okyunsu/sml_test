...
class MLServiceSettings(BaseServiceSettings):
    ...
    # === ML 모델 설정 (Material Service는 모델 사용 안함) ===
    MODEL_BASE_PATH: Optional[str] = Field(default=None, description="모델 기본 경로")
    MODEL_NAME: Optional[str] = Field(default=None, description="사용할 모델 이름")
    DISABLE_ML_MODEL: bool = Field(default=True, description="ML 모델 비활성화")
